...
class IfEvaluator(Evaluator):
    ...
    def evaluate(self) -> bool:
        ...
        if isinstance(self.node.test, ast.Compare): # If we have e.g. "if a == b"
            ...
            items = self.node.test.comparators # Not the LHS (list as we may have a == b == c or smth)
            ...
            print(f"Test type {type(self.node.test)} not supported. Currently supported types are ast.Compare and ast.Name.")
            ...
            for tgt in self.node.targets:
                if isinstance(tgt, ast.Name):
                    ...
                    if not value_labels.issubset(target_labels):
                        print(f"WARNING: Assigning {self.node.value.id} to {tgt.id}, where {tgt.id} has labels {target_labels} and the assigned value has labels {value_labels}.")
                        return False
                    # If target is missing any of the labels in PC, warn.
                    if not self.state.get_pc().issubset(target_labels):
                        print(f"WARNING: Assigning to variable {tgt.id} with labels {target_labels} despite PC being {self.state.get_pc}")
                else:
